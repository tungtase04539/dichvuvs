                            ...
                            {packages.map((pkg) => (
                                <div
                                    ...
                                    className={cn(
                                        ...
                                        pkg.id === "platinum"
                                            ? "bg-black border-[#00d4ff] shadow-[0_0_50px_rgba(0,212,255,0.25)] hover:shadow-[0_0_80px_rgba(0,212,255,0.4)]"
                                            : pkg.popular
                                                ? "bg-black border-[#ffcc00] shadow-[0_0_50px_rgba(255,204,0,0.25)] hover:shadow-[0_0_80px_rgba(255,204,0,0.4)]"
                                                : "bg-black border-white/20 hover:border-white shadow-2xl shadow-black/60",
                                        (pkg.id === "platinum" || pkg.popular) && "before:absolute before:inset-[-4px] before:rounded-[3.2rem] before:pointer-events-none before:z-[-1] before:content-['']",
                                        pkg.id === "platinum" && "before:bg-[#00d4ff] before:opacity-0 group-hover:before:opacity-100 before:blur-md before:transition-opacity",
                                        pkg.popular && "before:bg-[#ffcc00] before:opacity-0 group-hover:before:opacity-100 before:blur-md before:transition-opacity"
                                    )}
                                >
                                    {/* Background Pattern */}
                                    ...
                                    {pkg.popular && (
                                        <div className="absolute -top-6 left-1/2 -translate-x-1/2 bg-[#ffcc00] text-black px-8 py-2.5 rounded-full text-xs font-black uppercase tracking-widest shadow-2xl whitespace-nowrap z-20 border-2 border-white/20">
                                            Phổ biến nhất
                                        </div>
                                    )}
                                    {pkg.id === "platinum" && (
                                        <div className="absolute -top-6 left-1/2 -translate-x-1/2 bg-[#00d4ff] text-black px-8 py-2.5 rounded-full text-xs font-black uppercase tracking-widest shadow-2xl whitespace-nowrap z-20 border-2 border-white/20">
                                            VIP nhất
                                        </div>
                                    )}
                                    ...
                                    <div className="relative z-10 flex flex-col h-full">
                                        <div className="mb-10 flex items-center justify-between">
                                            <div className={cn("p-5 rounded-2xl", pkg.id === "platinum" ? "bg-[#00d4ff]/10" : pkg.popular ? "bg-[#ffcc00]/10" : "bg-white/5")}>
                                                {pkg.icon}
                                            </div>
                                            <div className="text-right">
                                                <div className={cn("text-xs font-black uppercase tracking-[0.2em] mb-2", pkg.id === "platinum" ? "text-[#00d4ff]" : "text-[#ffcc00]")}>{pkg.name}</div>
                                            ...
                                            <div className="space-y-5">
                                                {pkg.features.map((feature, i) => (
                                                    <div key={i} className="flex items-start gap-4 text-sm leading-tight">
                                                        <CheckCircle className={cn("w-6 h-6 shrink-0 mt-[-2px]", pkg.id === "platinum" ? "text-sky-400" : "text-amber-400", !pkg.popular && pkg.id !== "platinum" && "opacity-30")} />
                                        ...
                                        <button
                                            ...
                                            className={cn(
                                                ...
                                                pkg.id === "platinum"
                                                    ? "bg-white text-black hover:bg-sky-50 shadow-[0_0_30px_rgba(56,189,248,0.3)]"
                                                    : pkg.popular
                                                        ? "bg-amber-400 text-black hover:bg-amber-300 shadow-xl shadow-amber-400/20"
